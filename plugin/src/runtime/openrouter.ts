      ...
      try {
        const response = await fetch(`${this.baseUrl}/chat/completions`, {
          ...
          headers: {
            ...
            'HTTP-Referer': 'https://github.com/irl-dan/open-prose',
            'X-Title': 'OpenProse',
          },
      ...
      if (contextKeys.length > 0) {
        ...
        if (conversationKeys.length > 0) {
          ...
          conversationKeys.sort((a, b) => {
            const numA = parseInt(a.replace('conversation_turn_', ''));
            const numB = parseInt(b.replace('conversation_turn_', ''));
            return numA - numB;
          });
