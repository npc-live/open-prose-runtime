    ...
    if (entries.length > 0) {
      ...
      for (const [key, value] of entries) {
        ...
        if (typeof value === 'string') {
          prompt += `${value}\n\n`;
        } else if (typeof value === 'object' && value !== null && 'output' in value) {
          prompt += `${(value as any).output}\n\n`;
        } else {
...
export class CliProvider {
  ...
  constructor(provider: string) {
    ...
    if (!cfg) {
      throw new Error(
        ...
        `Add it to .open-prose.json or ~/.config/open-prose/providers.json`
      );
    }
    this.cfg = cfg;
  }
  ...
  async executeSession(
    ...
    const prompt = buildPrompt(spec, enableTools, allowedTools, skillPrompts);
